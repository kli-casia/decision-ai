...
DIR = os.path.abspath(os.path.dirname(__file__) + os.path.sep + "dqn_pri_model.pkl")

env = make_atari(args.env_id) if args.use_wrapper else gym.make(args.env_id)
env = wrap_Framestack(env, scale=False, frame_stack=True) if args.use_wrapper else env
frame = env.reset()
...
for i in range(frames):
    ...
    if i % print_interval == 0:
        ...
        capacity_pd.to_csv(joindir(RESULT_DIR, "capacity_record_dqn_pri.csv"))
        record_pd = pd.DataFrame(reward_record)
        record_pd.to_csv(joindir(RESULT_DIR, "dqn_pri_record_{}.csv".format(args.env_id)))
        start = time.time()
    if done:
        frame = env.reset()
        ...
        reward_record.append({"episode": episode_num, "meanreward": avg_reward})
g_end = time.time()
