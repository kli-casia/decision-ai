...
class DQN_Agent:
    def __init__(self, input_shape, action_space=None, USE_CUDA=False, memory_size=10000, prio_a=0.6, prio_e=0.001, epsilon=1, lr=1e-4, priority=True, mlp=False):
        ...
        self.DQN = DQN(input_shape=input_shape, num_actions=action_space.n)
        self.DQN_target = DQN(input_shape=input_shape,
    ...
    def compute_td_loss(self, idxs, states, actions, rewards, next_states, is_done, gamma=0.99):
        ...
        next_state_values = predicted_next_qvalues_target.gather(
            1, torch.max(predicted_next_qvalues_current, 1)[1].unsqueeze(1)).squeeze(1)
