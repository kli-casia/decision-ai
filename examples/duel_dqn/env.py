...
DIR = os.path.abspath(os.path.dirname(__file__) +
                      os.path.sep + "ddqn_model.pkl")

env = make_atari(args.env_id) if args.use_wrapper else gym.make(args.env_id)
env = wrap_Framestack(
    env, scale=False, frame_stack=True) if args.use_wrapper else env
frame = env.reset()
...
for i in range(frames):
    ...
    if i % print_interval == 0:
        ...
        capacity_pd.to_csv(joindir(RESULT_DIR, "capacity_record_duel_dqn.csv"))
        record_pd = pd.DataFrame(reward_record)
        record_pd.to_csv(
            joindir(RESULT_DIR, "duel_dqn_record_{}.csv".format(args.env_id)))
        start = time.time()
    if done:
        frame = env.reset()
