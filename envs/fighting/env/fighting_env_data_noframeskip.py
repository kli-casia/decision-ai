...
from msagent.envs.fighting.env.Machete import Machete
...
class Fightingice_Data_NoFrameskip(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, **kwargs):

        self.freq_restart_java = 30
        ...
        if self.system_name == "linux":
            ...
            print("Hey Bro, The java seems to work right, please make sure you can run java if you see some error")
    ...
    def _start_java_game(self):
        ...
        if self.system_name == "windows":
            ...
            # the --grey-bg means grey background
            self.java_env = subprocess.Popen(["java", "-Xms1024m", "-Xmx1024m", "-cp", self.start_up_str, "Main", "--port", str(self.port), "--disable-window","--py4j", "--fastmode",
                                           "--inverted-player", "1", "--mute", "--limithp", "400", "400"], stdout=devnull, stderr=devnull)
        elif self.system_name == "linux":
            self.java_env = subprocess.Popen(["java", "-cp", self.start_up_str, "Main", "--port", str(self.port), "--disable-window", "--py4j", "--fastmode",
                                            "--grey-bg", "--inverted-player", "1", "--mute", "--limithp", "400", "400"], stdout=devnull, stderr=devnull)
        elif self.system_name == "macos":
            self.java_env = subprocess.Popen(["java", "-XstartOnFirstThread", "-cp", self.start_up_str, "Main", "--port", str(self.port), "--disable-window", "--py4j", "--fastmode",
                                            "--grey-bg", "--inverted-player", "1", "--mute", "--limithp", "400", "400"], stdout=devnull, stderr=devnull)
...
if __name__ == "__main__":
    env = FightingiceEnv_Data_NoFrameskip(port=4245)
    ...
    while True:
        ...
        while not done:
            new_obs, reward, done, info = env.step(random.randint(0, 10))
            if info:
                print("asdfaf")
                print(type(info))
                print(info)
